...
export function Sidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  const isConsultServicesBrand = logoUrl === defaultLogoUrl;
  ...
  return (
    <>
      ...
      <aside className={cn(
        "w-64 bg-[var(--brand-primary)] text-white flex flex-col flex-shrink-0 z-50 h-full transition-transform duration-300",
        ...
        <div className="flex flex-col shrink-0 relative">
          <div className="h-28 bg-white rounded-b-[14px] border-b border-slate-200 relative z-10 w-full overflow-hidden flex items-center justify-center px-[18px] py-[14px]">
            <div className={cn(
              "h-24 w-24 shrink-0 overflow-hidden rounded-[14px] bg-white flex items-center justify-center",
              isConsultServicesBrand && "w-[188px] rounded-none"
            )}>
              <img
                src={logoUrl}
                alt={branding?.companyName || "Consult Services Tecnologia"}
                className={cn(
                  "block h-full w-full object-contain object-center rounded-[14px]",
                  isConsultServicesBrand && "h-[136%] w-[136%] max-w-none rounded-none"
                )}
              />
            </div>
          </div>
        ...
        <div className="px-6 pt-6 pb-2 flex flex-col gap-1 border-b border-white/10">
          ...
          <span className="text-sm font-bold text-white leading-tight mt-1">
            Gestão Jurídica e operações
          </span>
          <span className="text-[10px] text-white/70 mt-1 mb-2">
            Uma plataforma Consult Services Tecnologia
          </span>
          <div className="mt-1 mb-2 inline-flex border border-white/30 rounded-full px-3 py-1 items-center justify-center w-max">
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active 
                      ? "bg-white/10 border-r-4 border-white text-white"
                      : "opacity-70 hover:opacity-100 hover:bg-white/5 text-white"
                  )}
                >
